...
func build(owner_ui: Control, bounds: Vector2) -> void:
	...
	var footer_h := 46.0
	var hand_h := clampf(h * 0.19, 98, 150)
	...
	if wide:
		...
		var foundation_h := clampf(h * 0.16, 55, 118)
		if small:
			var hand_width := 140.0
		...
		var opponent_h := clampf(h * 0.086, 44, 64) + 22
		...
		var foundation_h := clampf(h * 0.09, 45, 78)
...
func _foundations(area: Rect2) -> void:
	var gap := 12.0
	var ch := area.size.y
	var cw := minf(ch * 0.71, (area.size.x - 3 * gap) / 4)
...
func _rows(area: Rect2) -> void:
	var gap := clampf(area.size.x * 0.025, 6, 24)
	var cw := minf(116, (area.size.x - gap * 4) / 5)
	var ch := minf(cw * 1.48, maxf(55, area.size.y - 24))
	cw = minf(cw, ch / 1.38)
	...
	for row in range(5):
		...
		var step := minf(27, (available - ch) / maxi(1, visible - 1))
		...
		if pile.size() > visible or pile.size() > 1:
			var inspect: Button = app._button(str(pile.size()) + " cards", func(): app._inspect_row(row))
			inspect.add_theme_font_size_override("font_size", 11)
			place(inspect, Rect2(x, y + available - 24, cw, 24))
...
func _hand(area: Rect2) -> void:
	...
	label_at(title, Rect2(area.position, Vector2(area.size.x, 20)), 14, GOLD)
	var sources := ["play", "discard"]
	if p.reserve_count > 0: sources.append("reserve")
	if p.held >= 0: sources.append("held")
	var gap := 14.0
	var ch := maxf(48, area.size.y - 40)
	var cw := minf(ch * 0.72, (area.size.x - (sources.size() - 1) * gap) / sources.size())
	ch = minf(ch, cw * 1.48)
	...
	for i in range(sources.size()):
		...
		var rect := Rect2(start + i * (cw + gap), area.position.y + 22, cw, ch)
...
func _actions(area: Rect2, small: bool) -> void:
	...
	for i in range(items.size()):
		...
		button.add_theme_font_size_override("font_size", 14 if small else 16)
